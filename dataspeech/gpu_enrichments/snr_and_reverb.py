    ...
    if isinstance(batch[audio_column_name], list):  
        ...
        for sample in batch[audio_column_name]:
            try:
                ...
                snr.append(float(res["snr"][mask].mean()))
                c50.append(float(res["c50"][mask].mean()))
                vad_durations.append(float(vad_duration))
            except:
                snr.append(0.0)
                c50.append(0.0)
                vad_durations.append(0.0)
        ...
        try:
            ...
            batch["snr"] = float(res["snr"][mask].mean())
            batch["c50"] = float(res["c50"][mask].mean())
            batch["speech_duration"] = float(vad_duration)
        except:
            batch["snr"] = 0.0
            batch["c50"] = 0.0
            batch["speech_duration"] = 0.0
