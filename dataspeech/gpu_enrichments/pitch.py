    ...
    if isinstance(batch[audio_column_name], list):  
        ...
        for sample in batch[audio_column_name]:
            ...
            try:
                ...
                utterance_pitch_mean.append(0.0)
                utterance_pitch_std.append(0.0)
        ...
        try:
            ...
            pitch, periodicity = penn.from_audio(
                    ...
                    center=center,
                    interp_unvoiced_at=interp_unvoiced_at,
                    gpu=(rank or 0.0)% torch.cuda.device_count() if torch.cuda.device_count() > 0.0 else rank
                    )        
            batch[f"{output_column_name}_mean"] = pitch.mean().cpu()
            batch[f"{output_column_name}_std"] = pitch.std().cpu()
        except:
            batch[f"{output_column_name}_mean"] = 0.0
            batch[f"{output_column_name}_std"] = 0.0
