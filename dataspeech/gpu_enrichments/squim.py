...
def squim_apply(batch, rank=None, audio_column_name="audio"):
    ...
    if isinstance(batch[audio_column_name], list):  
        ...
        for sample in batch[audio_column_name]:
            try:
                waveform = torchaudio.functional.resample(torch.tensor(sample["array"])[None, :].to(device).float(), 16000, SQUIM_OBJECTIVE.sample_rate)
                with torch.no_grad():
        ...
        try:
            waveform = torchaudio.functional.resample(torch.tensor(batch[audio_column_name]["array"][None, :]).to(device).float(), 16000, SQUIM_OBJECTIVE.sample_rate)
            with torch.no_grad():
                stoi_sample, pesq_sample, sdr_sample = model(waveform)
